...
class TestLayeringTest {
  ...
  void unitLayerTestsMustNotDependOnIntegrationLayerBase() {
    JavaClasses classes = new ClassFileImporter().importPackages("org.folio");

    List<String> violations = classes.stream()
      .filter(javaClass -> !javaClass.getPackageName().startsWith("org.folio.it"))
      .filter(javaClass -> javaClass.getDirectDependenciesFromSelf().stream()
        .anyMatch(dependency -> dependency.getTargetClass().getFullName().equals(INTEGRATION_LAYER_BASE)))
