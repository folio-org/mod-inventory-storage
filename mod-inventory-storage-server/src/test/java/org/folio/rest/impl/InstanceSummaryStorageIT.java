...
class InstanceSummaryStorageIT extends BaseIntegrationTest {
  ...
  void shouldReturn404_whenInstanceDoesNotExist() {
    var response = get(doGet(client, ResourcePaths.INSTANCES + "/" + UUID.randomUUID() + "/summary"));
  ...
  private static String createInstance(String title, JsonObject extraFields) {
    ...
    return get(doPost(client, ResourcePaths.INSTANCES, request)).jsonBody().getString(ID_FIELD);
  }
  ...
  private static void createBoundWithPart(String holdingsRecordId, String itemId) {
    get(doPost(client, ResourcePaths.BOUND_WITH_PARTS, new JsonObject()
  ...
  private static String createInstanceFormat(String name) {
    ...
    get(doPost(client, ResourcePaths.INSTANCE_FORMATS, pojo2JsonObject(instanceFormat)));
    return id;
  }
  ...
  private static String createModeOfIssuance(String name) {
    ...
    get(doPost(client, ResourcePaths.MODES_OF_ISSUANCE, pojo2JsonObject(issuanceMode)));
    return id;
  }
  ...
  private static String createNatureOfContentTerm(String name) {
    ...
    get(doPost(client, ResourcePaths.NATURE_OF_CONTENT_TERMS, pojo2JsonObject(natureOfContentTerm)));
    return id;
  }

  private static JsonObject getSummary(String instanceId) {
    var response = get(doGet(client, ResourcePaths.INSTANCES + "/" + instanceId + "/summary"));
