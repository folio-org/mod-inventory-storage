...
class HoldingsStorageIT extends BaseIntegrationTest {
  ...
  void shouldDeleteHolding() {
    ...
    assertThat(get(doDelete(client, ResourcePaths.HOLDINGS + "/" + holdingId)).status()).isEqualTo(SC_NO_CONTENT);
  ...
  void shouldGetAllHoldings() {
    ...
    var response = get(doGet(client, ResourcePaths.HOLDINGS)).jsonBody();
  ...
  void shouldRetrieveAllHoldings() {
    ...
    var response = get(doPost(client, ResourcePaths.HOLDINGS_RETRIEVE, new JsonObject())).jsonBody();
  ...
  void shouldReturn400_whenLimitIsNegative() {
    ...
    var response = get(doGet(client, ResourcePaths.HOLDINGS + "?limit=-3"));
  ...
  void shouldReturn400_whenOffsetIsNegative() {
    ...
    var response = get(doGet(client, ResourcePaths.HOLDINGS + "?offset=-3"));
  ...
  void shouldPageThroughAllHoldings() {
    ...
    var firstPage = get(doGet(client, ResourcePaths.HOLDINGS + "?limit=3")).jsonBody();
    var secondPage = get(doGet(client, ResourcePaths.HOLDINGS + "?limit=3&offset=3")).jsonBody();
  ...
  void shouldDeleteAllHoldings() {
    ...
    assertThat(get(doDelete(client, ResourcePaths.HOLDINGS + "?query=cql.allRecords=1")).status())
  ...
  void shouldDeleteHoldings_matchingCqlQuery() {
    ...
    var response = get(doDelete(client, ResourcePaths.HOLDINGS + "?query=hrid==12*"));
  ...
  void shouldReturn400_whenDeletingHoldingsWithEmptyCql() {
    var response = get(doDelete(client, ResourcePaths.HOLDINGS + "?query="));
  ...
  void shouldReturn400_whenTenantMissingForCreatingHolding() {
    ...
    var response = get(doPost(client, ResourcePaths.HOLDINGS, null, request));
  ...
  void shouldReturn400_whenTenantMissingForGettingHolding() {
    var response = get(doGet(client, ResourcePaths.HOLDINGS + "/" + UUID.randomUUID(), null));
  ...
  void shouldReturn400_whenTenantMissingForGettingAllHoldings() {
    var response = get(doGet(client, ResourcePaths.HOLDINGS, null));
  ...
  void shouldReturn422_whenCreatingHoldingWithDuplicateHrid() {
    ...
    var response = get(doPost(client, ResourcePaths.HOLDINGS, duplicate));
  ...
  void shouldReturn500_whenHridGenerationFails() {
    ...
    var response = get(doPost(client, ResourcePaths.HOLDINGS, holdingRequest(instanceId).create()));
  ...
  void shouldCreateHolding_viaSynchronousBatchWithoutIdAndUpsertTrue() {
    ...
    var found = get(doGet(client, ResourcePaths.HOLDINGS + "?query=callNumber=test-call-number")).jsonBody()
  ...
  void shouldReturn400_whenDeletingHoldingsSourceStillAttached() {
    ...
    var response = get(doDelete(client, ResourcePaths.HOLDINGS_SOURCES + "/" + sourceId));
  ...
  private static TestResponse updateHolding(JsonObject holding) {
    return get(doPut(client, ResourcePaths.HOLDINGS + "/" + holding.getString("id"), holding));
  }
  ...
  private static TestResponse getHoldingById(String id) {
    return get(doGet(client, ResourcePaths.HOLDINGS + "/" + id));
  }
  ...
  private static void assertExists(JsonObject expectedHolding, String tenantId) {
    var response = get(doGet(client, ResourcePaths.HOLDINGS + "/" + expectedHolding.getString("id"), tenantId));
  ...
  private static TestResponse syncBatch(String queryParams, JsonArray holdingsArray) {
    return get(doPost(client, ResourcePaths.HOLDINGS_SYNC + queryParams,
  ...
  private static JsonObject searchForHoldings(String cql) {
    return get(doGet(client, ResourcePaths.HOLDINGS + "?query=" + urlEncode(cql))).jsonBody();
  }
