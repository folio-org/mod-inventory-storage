...
class SettingStorageIT extends BaseIntegrationTest {
  ...
  private void restoreAndAwait(String tenantId) {
    var settingId = get(doGet(client, SETTING_PATH, tenantId)).jsonBody().getString(ID_FIELD);
    ...
    var response = get(doPatch(client, SETTING_PATH, tenantId, updateRequest(DEFAULT_SETTING_VALUE)));
  ...
  void shouldReturnSetting_whenKeyExists() {
    var response = get(doGet(client, SETTING_PATH));
  ...
  void shouldReturn404_whenGettingNonExistentSetting() {
    var response = get(doGet(client, ResourcePaths.INVENTORY_SETTINGS + "/" + NON_EXISTENT_KEY));
  ...
  void shouldReturnConsistentType_acrossMultipleGets() {
    var first = get(doGet(client, SETTING_PATH)).jsonBody();
    var second = get(doGet(client, SETTING_PATH)).jsonBody();
  ...
  void shouldUpdateSettingValue_andPublishEvent() {
    var settingId = get(doGet(client, SETTING_PATH)).jsonBody().getString(ID_FIELD);

    assertThat(get(doPatch(client, SETTING_PATH, updateRequest(false))).status()).isEqualTo(SC_NO_CONTENT);
    settingEventMessageChecks.settingEventPublished(new SettingEvent(settingId, SETTING_KEY, false, TENANT_ID));
    assertThat(get(doGet(client, SETTING_PATH)).jsonBody().getString(VALUE_FIELD)).isEqualTo("false");

    KAFKA_CONSUMER.discardAllMessages();

    assertThat(get(doPatch(client, SETTING_PATH, updateRequest(true))).status()).isEqualTo(SC_NO_CONTENT);
    settingEventMessageChecks.settingEventPublished(new SettingEvent(settingId, SETTING_KEY, true, TENANT_ID));
    assertThat(get(doGet(client, SETTING_PATH)).jsonBody().getString(VALUE_FIELD)).isEqualTo("true");
  }
  ...
  private void updateAndVerify(String settingId, boolean value) {
    assertThat(get(doPatch(client, SETTING_PATH, updateRequest(value))).status()).isEqualTo(SC_NO_CONTENT);
    settingEventMessageChecks.settingEventPublished(new SettingEvent(settingId, SETTING_KEY, value, TENANT_ID));
    assertThat(get(doGet(client, SETTING_PATH)).jsonBody().getString(VALUE_FIELD)).isEqualTo(String.valueOf(value));
    KAFKA_CONSUMER.discardAllMessages();
  }
  ...
  void shouldKeepSameId_afterUpdate() {
    var before = get(doGet(client, SETTING_PATH)).jsonBody();
    var idBefore = before.getString(ID_FIELD);

    get(doPatch(client, SETTING_PATH, updateRequest(!Boolean.parseBoolean(before.getString(VALUE_FIELD)))));
    settingEventMessageChecks.settingEventPublished(idBefore);

    var idAfter = get(doGet(client, SETTING_PATH)).jsonBody().getString(ID_FIELD);
  ...
  void shouldReturn422_whenUpdatingWithInvalidType() {
    var response = get(doPatch(client, SETTING_PATH, new JsonObject().put(VALUE_FIELD, "not a boolean")));
  ...
  void shouldReturn422_whenUpdatingWithNullValue() {
    var response = get(doPatch(client, SETTING_PATH, new JsonObject().putNull(VALUE_FIELD)));
  ...
  void shouldReturn404_whenUpdatingNonExistentSetting() {
    var response = get(doPatch(client, ResourcePaths.INVENTORY_SETTINGS + "/" + NON_EXISTENT_KEY,
      updateRequest(true)));
  ...
  void shouldUpdateSettingAndPublishEvent_forConsortiumCentralTenant() {
    var initial = get(doGet(client, SETTING_PATH, CONSORTIUM_CENTRAL_TENANT));
    ...
    var updateResponse = get(doPatch(client, SETTING_PATH, CONSORTIUM_CENTRAL_TENANT, updateRequest(newValue)));
    assertThat(updateResponse.status()).isEqualTo(SC_NO_CONTENT);
    settingEventMessageChecks.settingEventPublished(settingId);

    var getResponse = get(doGet(client, SETTING_PATH, CONSORTIUM_CENTRAL_TENANT));
  ...
  void shouldReturn400_whenUpdatingFromConsortiumMemberTenant() {
    var initial = get(doGet(client, SETTING_PATH, CONSORTIUM_MEMBER_TENANT));
    assertThat(initial.status()).isEqualTo(SC_OK);
    var initialValue = initial.jsonBody().getString(VALUE_FIELD);
    var newValue = !"true".equals(initialValue);

    var updateResponse = get(doPatch(client, SETTING_PATH, CONSORTIUM_MEMBER_TENANT, updateRequest(newValue)));
    assertThat(updateResponse.status()).isEqualTo(SC_BAD_REQUEST);

    var getResponse = get(doGet(client, SETTING_PATH, CONSORTIUM_MEMBER_TENANT));
    assertThat(getResponse.status()).isEqualTo(SC_OK);
  ...
  void shouldPublishEvent_withCentralTenantId_whenCentralTenantUpdates() {
    var initial = get(doGet(client, SETTING_PATH, CONSORTIUM_CENTRAL_TENANT)).jsonBody();
  ...
  void shouldPublishEventForMemberTenant_whenCentralTenantUpdates() {
    var memberSettingId = get(doGet(client, SETTING_PATH, CONSORTIUM_MEMBER_TENANT)).jsonBody().getString(ID_FIELD);
    var central = get(doGet(client, SETTING_PATH, CONSORTIUM_CENTRAL_TENANT)).jsonBody();
