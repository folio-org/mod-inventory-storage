...
import static org.folio.it.LocationStorageFixtures.createCampus;
import static org.folio.it.LocationStorageFixtures.createInstitution;
...
import org.folio.support.builders.BoundWithPartBuilder;
...
class InventoryHierarchyViewIT extends BaseIntegrationTest {
  ...
  void shouldUseTemporaryLocation_asEffectiveLocation() {
    ...
    assertThat(await(doPut(client, ResourcePaths.HOLDINGS + "/" + holdingId, holding)).status())
  ...
  private static String createItem(ItemRequestBuilder builder) {
    var response = await(doPost(client, ResourcePaths.ITEMS, builder.create()));
  ...
  private static void createBoundWithPart(String holdingId, String itemId) {
    ...
    var response = await(doPost(client, ResourcePaths.BOUND_WITH_PARTS, request));
    assertThat(response.status()).isEqualTo(SC_CREATED);
  }

  private static JsonObject getJson(String path) {
    return await(doGet(client, path)).jsonBody();
  }
  ...
  private static TestResponse requestUpdatedInstanceIdsRaw(Map<String, String> queryParams) {
    ...
    return await(doGet(client, ResourcePaths.INVENTORY_HIERARCHY_UPDATED_INSTANCE_IDS + "?" + query));
  }
  ...
  private static TestResponse requestItemsAndHoldingsRaw(List<String> instanceIds, boolean skipSuppressed) {
    ...
    return await(doPost(client, ResourcePaths.INVENTORY_HIERARCHY_ITEMS_AND_HOLDINGS, pojo2JsonObject(payload)));
  }
