...
import static org.folio.support.ResourcePaths.PRECEDING_SUCCEEDING_TITLES;
...
class PrecedingSucceedingTitlesIT extends BaseIntegrationTest {
  ...
  void shouldDeleteTitle() {
    ...
    assertThat(await(doDelete(client, titleByIdPath(id))).status()).isEqualTo(SC_NO_CONTENT);
    assertThat(await(doGet(client, titleByIdPath(id))).status()).isEqualTo(SC_NOT_FOUND);
  }
  ...
  void shouldFindTitleByQuery() {
    ...
    var response = await(
      doGet(client, ResourcePaths.PRECEDING_SUCCEEDING_TITLES + "?query=succeedingInstanceId=" + succeedingId));
  ...
  void shouldUpdateTitleCollection_forInstance() {
    ...
    var found = await(doGet(client, ResourcePaths.PRECEDING_SUCCEEDING_TITLES
                                                  + "?query=succeedingInstanceId==(" + instanceId
                                                  + ")+or+precedingInstanceId==(" + instanceId + ")"))
  ...
  void shouldReturn404_whenUpdatingTitlesForMissingInstance() {
    ...
    var response =
      await(doPut(client, ResourcePaths.PRECEDING_SUCCEEDING_TITLES + "/instances/" + missingInstanceId,
        pojo2JsonObject(updated)));
  ...
  private TestResponse postTitle(String precedingId, String succeedingId, String title, String hrid,
    ...
    return await(doPost(client, PRECEDING_SUCCEEDING_TITLES, pojo2JsonObject(request)));
  }
  ...
  private void replaceTitle(String id, String precedingId, String succeedingId, String title, String hrid,
    ...
    var response = await(doPut(client, titleByIdPath(id), pojo2JsonObject(request)));
    assertThat(response.status()).isEqualTo(SC_NO_CONTENT);
  }

  private JsonObject getTitleById(String id) {
    return await(doGet(client, titleByIdPath(id))).jsonBody();
  }
