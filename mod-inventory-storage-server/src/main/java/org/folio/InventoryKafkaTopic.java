...
public enum InventoryKafkaTopic implements KafkaTopic {
   ...
   */
  private static final Map<InventoryKafkaTopic, Pair<String, Integer>> TOPIC_PARTITION_MAP;

  private static final Map<InventoryKafkaTopic, Pair<String, Integer>> TOPIC_MESSAGE_RETENTION_MAP = Map.of(
    REINDEX_RECORDS, Pair.of("KAFKA_REINDEX_RECORDS_TOPIC_MESSAGE_RETENTION", 86400000) // 1 day
  );

  private static final Map<InventoryKafkaTopic, Pair<String, Integer>> TOPIC_MESSAGE_MAX_SIZE_MAP = Map.of(
    REINDEX_RECORDS, Pair.of("KAFKA_REINDEX_RECORDS_TOPIC_MAX_MESSAGE_SIZE", 10485760) // 10 MB
  );

  static {
    var map = new EnumMap<InventoryKafkaTopic, Pair<String, Integer>>(InventoryKafkaTopic.class);
    ...
    map.put(SETTING, Pair.of("KAFKA_SETTING_TOPIC_NUM_PARTITIONS", 1));
  ...
  public int numPartitions() {
    return Optional.ofNullable(TOPIC_PARTITION_MAP.get(this))
      .map(pair -> getIntValue(pair.getKey(), pair.getValue()))
      .orElse(Environment.getIntValue(DEFAULT_NUM_PARTITIONS_PROPERTY, DEFAULT_NUM_PARTITIONS_VALUE));
  }
