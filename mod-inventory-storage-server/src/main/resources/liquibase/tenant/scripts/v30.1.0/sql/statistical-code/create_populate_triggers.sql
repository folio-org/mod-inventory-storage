...
BEGIN
    ...
    IF (TG_OP = 'UPDATE') THEN
        DELETE
        FROM instance_statistical_code
        WHERE instance_id = OLD.id;
    END IF;

    -- Insert new entries for the instance
    INSERT INTO instance_statistical_code (instance_id, statistical_code_id)
...
BEGIN
    ...
    IF (TG_OP = 'UPDATE') THEN
        DELETE
        FROM item_statistical_code
        WHERE item_id = NEW.id;
    END IF;

    -- Insert new entries for the item
    INSERT INTO item_statistical_code (item_id, statistical_code_id)
    SELECT NEW.id, jsonb_array_elements_text(NEW.jsonb->'statisticalCodeIds')::uuid;
...
BEGIN
    ...
    IF (TG_OP = 'UPDATE') THEN
        DELETE
        FROM holdings_record_statistical_code
        WHERE holdings_record_id = NEW.id;
    END IF;

    -- Insert new entries for the holdings_record
    INSERT INTO holdings_record_statistical_code (holdings_record_id, statistical_code_id)
    SELECT NEW.id, jsonb_array_elements_text(NEW.jsonb->'statisticalCodeIds')::uuid;
