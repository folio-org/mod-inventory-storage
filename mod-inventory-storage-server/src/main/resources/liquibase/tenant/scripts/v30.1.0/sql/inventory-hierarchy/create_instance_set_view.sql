CREATE OR REPLACE VIEW instance_set
AS
SELECT id,
       ...
       ( SELECT COALESCE(jsonb_agg(item.jsonb), '[]'::jsonb) AS "coalesce"
         FROM holdings_record hr
                  JOIN item ON item.holdingsrecordid = hr.id AND hr.instanceid = instance.id) AS items,
       ( SELECT COALESCE(jsonb_agg(preceding_succeeding_title.jsonb), '[]'::jsonb) AS "coalesce"
         FROM preceding_succeeding_title
