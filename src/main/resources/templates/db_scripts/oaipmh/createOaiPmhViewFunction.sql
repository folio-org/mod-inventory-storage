...
with instanceIdsInRange as ( select inst.id                                              as instanceId,
                             ...
                             where ((hr.jsonb #>> '{metadata,updatedDate}')::timestamp between dateOrMin($1) and dateOrMax($2) or
                             ...
                             select (audit_holdings_record.jsonb #>> '{record,instanceId}')::uuid,
                                    greatest((audit_item.jsonb #>> '{record,updatedDate}')::timestamp,
                                             (audit_holdings_record.jsonb #>> '{record,updatedDate}')::timestamp) as maxDate
                             ...
                             where ((audit_holdings_record.jsonb #>> '{record,updatedDate}')::timestamp between dateOrMin($1) and dateOrMax($2) or
