...
public class SettingsRepository extends AbstractRepository<Setting> {
  ...
  private Setting mapToSetting(Row row) {
    return new Setting()
      ...
      .withCreatedByUserId(row.getUUID("created_by_user_id"))
      .withCreatedDate(toDate(row.getLocalDateTime("created_date")))
      .withUpdatedByUserId(row.getUUID("updated_by_user_id"))
      .withUpdatedDate(toDate(row.getLocalDateTime("updated_date")));
  }
