    ...
    vertxContext.runOnContext(v -> {
      try {
        ...
        PostgresClient.getInstance(vertxContext.owner(), tenantId)
                .get(
            LOCATION_TABLE, Location.class, new String[]{"*"},                cql, true, true, reply -> {
          try {
            if(reply.failed()) {
          ...
          try {
            if(reply.failed()) {
              ...
              if(isDuplicate(message)) {
                asyncResultHandler.handle(Future.succeededFuture(
                  PostLocationsResponse.withJsonUnprocessableEntity(                                ValidationHelper.createValidationErrorMessage(
