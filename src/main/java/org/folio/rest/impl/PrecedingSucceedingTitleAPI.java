...
public class PrecedingSucceedingTitleAPI implements org.folio.rest.jaxrs.resource.PrecedingSucceedingTitles {
  ...
  public void postPrecedingSucceedingTitles(String lang, PrecedingSucceedingTitle entity,
    Map<String, String> okapiHeaders, Handler<AsyncResult<Response>> asyncResultHandler, Context vertxContext) {
    if (precedingOrSucceedingInstanceIdIsEmpty(entity)) {
      handleError(asyncResultHandler);
      return;
    }
  ...
  private boolean precedingOrSucceedingInstanceIdIsEmpty(PrecedingSucceedingTitle entity) {
    return ObjectUtils.isEmpty(entity.getPrecedingInstanceId()) && ObjectUtils.isEmpty(entity.getSucceedingInstanceId());
  }

  private void handleError(Handler<AsyncResult<Response>> asyncResultHandler) {
    asyncResultHandler.handle(Future.succeededFuture(
      PostPrecedingSucceedingTitlesResponse.respond422WithApplicationJson(
        ValidationHelper.createValidationErrorMessage("", "", "The precedingInstanceId and succeedingInstanceId can't be empty at the same time"))));
  }
}
